...
--
-- Hôte : 127.0.0.1:3306
-- Généré le : jeu. 27 mai 2021 à 21:18
...
DROP PROCEDURE IF EXISTS `sp_InsertCommande`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_InsertCommande` (IN `unId` INT(50))  NO SQL
    COMMENT 'Insertion d''une nouvelle commande avec la date actuelle et le nu'
insert into commande(`DateCmde`,`NumClient`) values(now(),unId)$$

DROP PROCEDURE IF EXISTS `sp_InsertPortefeuille`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_InsertPortefeuille` (IN `unNomClient` VARCHAR(50), IN `uneAdresseClient` VARCHAR(50))  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_InsertProspect` (IN `unNomClient` VARCHAR(50), IN `uneAdresseClient` VARCHAR(50), IN `unNbVisites` INT)  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_transmuter` (IN `unNumClient` INT)  BEGIN
	Update client
